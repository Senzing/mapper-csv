        ...
        if line:  # --skip empty lines
            ...
            if fileInfo["fieldDelimiter"] == "JSON":
                ...
                for attr in csv_data:
                    if (
                        type(csv_data[attr]) not in (list, dict)
                        and attr not in fileInfo["header"]
                    ):
            ...
            if type(line) == list:
                ...
                row = [
                    ...
                    for x in line.split(fileInfo["fieldDelimiter"])
                ]
...
def analyzeFile():
        ...
        while rowData:
            ...
            for columnName in currentFile["header"]:
                ...
                if columnName not in mappingDoc["input"]["columnHeaders"]:
                    mappingDoc["input"]["columnHeaders"].append(
                        columnName
                    )  # --may not be if json

                columnValue = str(rowData[columnName]).strip()
                if not columnValue or columnValue.strip().upper() in (
                    "NONE",
                    "NULL",
                    "\\N",
                ):
    ...
    if outputFileName:
        ...
        outputFileWriter = csv.writer(
            outputFileHandle, dialect=csv.excel, quoting=csv.QUOTE_ALL
        )
    ...
    for columnName in mappingDoc["input"]["columnHeaders"]:
        ...
        topValue = []
        for value in sorted(
            statPack[columnName].items(), key=lambda x: x[1], reverse=True
        ):
            if value[0] != "null":
        ...
        if recordCount:
            ...
            columnMapping["statistics"]["top5values"] = [
                item for item in topValue if item
            ]
            ...
            for line in f:
                ...
                if line.strip() == "input_file = '<input_file_name>'":
                    ...
                    if mappingDoc["input"]["csvDialect"] == "pipe":
                        ...
                        registerDialect = (
                            "csv.register_dialect('other', delimiter = '"
                            + mappingDoc["input"]["fieldDelimiter"]
                            + "')"
                        )
                    else:
                    ...
                    if registerDialect:
                        codeLines.append(
                            (" " * (len(line) - len(line.strip()) - 1))
                            + registerDialect
                            + "\n"
                        )
                    codeLines.append(
                        line.replace("<dialect>", mappingDoc["input"]["csvDialect"])
                    )

                elif (
                    line.strip() == "input_file_handle = open(args.input_file, 'r')"
                    and mappingDoc["input"]["fileEncoding"]
                ):
                    codeLines.append(
                        line.replace(
                            ")",
                            ", encoding='%s')" % mappingDoc["input"]["fileEncoding"],
                        )
                    )
                    ...
                    if not bestRecordID.startswith("<"):
                        line = line.replace(
                            "'<supply>'", "raw_data['%s']" % bestRecordID
                        )
                    ...
                    for columnMapping in possibleMappings:
                        ...
                        codeLines.append(
                            "        # %s populated, %s unique\n"
                            % (
                                columnMapping["statistics"]["populated%"],
                                columnMapping["statistics"]["unique%"],
                            )
                        )
                        for item in columnMapping["statistics"]["top5values"]:
                            codeLines.append("        #      %s\n" % item)
                        codeLines.append(
                            "        json_data['%s'] = raw_data['%s']\n"
                            % (
                                columnMapping["statistics"]["columnName"],
                                columnMapping["statistics"]["columnName"],
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="optional name of a mapping file to generate",
    )
    parser.add_argument(
        "-p",
        "--pythonModuleFile",
