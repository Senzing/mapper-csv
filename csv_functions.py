...
class csv_functions:
    ...
    def __init__(self):
        ...
        try:
            self.variantJson = json.load(
                open(self.variantFile, "r", encoding="latin-1")
            )
        ...
        keys = ["GARBAGE_VALUES", "SENZING_ATTRIBUTES"]
        for key in keys:
        ...
        for variantList in self.variantJson:
            if type(self.variantJson[variantList]) == list:
                self.variantData[variantList] = dict(
                    zip(
                        [i.upper() for i in self.variantJson[variantList]],
                        [""] * len(self.variantJson[variantList]),
                    )
                )
            else:
    ...
    def format_date(self, dateString, outputFormat=None):
        for dateFormat in self.dateFormats:
            try:
                dateValue = datetime.strptime(dateString, dateFormat)
            except:
                pass
            else:
                if not outputFormat:
                ...
                if ok.upper().startswith("Y"):
                    ...
                    for variantList in csvFunctions.variantJson:
                        if type(csvFunctions.variantJson[variantList]) == list:
                            csvFunctions.variantJson[variantList] = sorted(
                                csvFunctions.variantJson[variantList]
                            )
                    # --write to file
