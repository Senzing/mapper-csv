...
class mapper:
    ...
    def map(self, raw_data, input_row_num=None):
        ...
        pii_attrs = ["name", "gender", "dob", "ssn", "addr1", "city", "state", "zip"]
        ...
        is_organization = self.is_organization(
            raw_data["name"], raw_data["dob"], raw_data["ssn"]
        )
    ...
    def is_organization(self, raw_name, raw_dob, raw_ssn):
        ...
        for token in (
            ...
            if token in self.variant_data["ORGANIZATION_TOKENS"]:
                ...
                " ".join(prior_tokens[-2:]) in self.variant_data["ORGANIZATION_TOKENS"]
            ):
                return True
            elif (
                " ".join(prior_tokens[-3:]) in self.variant_data["ORGANIZATION_TOKENS"]
            ):
                return True
