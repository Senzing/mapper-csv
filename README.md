...
_Having the csv_analyzer.py create a standalone python script is the **recommended method** as it adds more flexibility and is easier to customize when the raw csv data does not have direct mappings
to Senzing attributes. While it does require some knowledge of python, it is not much more complicated than the json mapping file approach which does not._

Please refer back to the [Generic entity specification] for more information on what
...
- [input/test_set1.csv]
- [mappings/test_set1.map]
- [mappings/test_set1.py]
- [output/test_set1.json]
...
_Note: The csv analyzer use the csv module sniffer to determine the file delimiter for you. If you have problems with this, you can override the delimiter and even the file encoding._
...
_Note: Normally you would decide if you want a simple mapping with the -m parameter or a python module with the -p parameter. There is no need to do both. Non-python programmers
can do simple mappings using the -m mapping file method. Python programmers will likely want to use the -p python module method as they have more complete control over the process._
...
Review the [mappings/test_set1.py]. It was built by the csv_analyzer which incorporates the columns in the csv file into the python_template.py file. The next step is to
...
_While not strictly necessary, when there is an obvious mapping it should be used._
...
_There is no need to map columns twice!_
...
_See the **"Attributes for names ..."** chapter of the [Generic entity specification] for
information on why it was mapped this way._
...
_See the **"Special attribute types and labels"** chapter of the [Generic entity specification]
for information on why it was mapped this way._
...
the [Generic entity specification]
...
_input_row_num is computed for you and passed into the map() class_

_This is fine option if the file will **never** be reloaded_
...
_The purpose of only including the fields used for resolution in the record_hash is so that any dates or status that may change will not create a duplicate record_

_This is the safest method if the file will ever be updated and reloaded!_
...
If the code you need to execute is more than a single line, create a function for it in the csv_functions.py script. Notice how the first calculation that creates is_organization calls the appropriate csv_function.

The calculations above determine whether or not the name field should be mapped to name_org or name_full and if the address should be labeled a "business" address. It is a best practice for organizations to have their name mapped as a name_org and their physical address to be labeled business.
...
_You may notice the statistics section supplied for each column by the csv_analyzer. While not required by the csv_mapper, it is included to help you decide on the appropriate mapping. For instance if this field was not populated or did not contain M and F as values, you may want to not map it at all or convert it in the calculations section described below._
...
_You must first update the Senzing configuration with any new data source codes, features or attributes you specified in your mapper. These can be added via
the G2ConfigTool python script provided in the install._
...
[Mapping file tutorial]: #mapping-file-tutorial
...
[Running the python module standalone]: #Running-the-python-module-standalone
